...
def send_activation_mail(email, activation_code):
    ...
    send_mail('Активация аккаунта', message, 'test@mail.com', [email])
...
class ChangePasswordForm(forms.Form):
    old_pass = forms.CharField(min_length=8,
                               widget=forms.PasswordInput)
    new_pass = forms.CharField(min_length=8,
                               widget=forms.PasswordInput)
    new_pass_confirm = forms.CharField(min_length=8,
                                       widget=forms.PasswordInput)
    ...
    def clean_old_pass(self):
        ...
        if not user.check_password(old_pass):
            raise forms.ValidationError('Укажите верынй пароль')
        return old_pass

    def clean(self):
        new_password = self.cleaned_data.get('new_pass')
